class Calender
  ...
  def puts_calender_days #カレンダーはここで出力
    uru_year?
    howmany_days 
    zeller
    ...
    Month_days.each do |print_day|    #うるう年でない2月のパターンを忘れていたため、この式になった
      ...
      case @month_28or30or31
      when 0 then
        if counter == 28 + @zeller_day && !uru_year?
          break
        elsif counter == 29 + @zeller_day && uru_year?
          break
        end
        
      when 1 then
        if counter == 30 + @zeller_day
          break
        end
        
      when 2 then
        if counter == 31 + @zeller_day
          break
        end
      end
...
calender.puts_calender_days #カレンダーの日付欄より上はここで出力
